...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    """
    numTimeSteps = 300
    totalPops = [0 for i in range(numTimeSteps)]
    for i in range(numTrials):
        p = Patient([SimpleVirus(maxBirthProb, clearProb) for v in range(numViruses)], maxPop)
        for timestep in range(numTimeSteps):
            p.update()
            totalPops[timestep] += p.getTotalPop()
    avgPops = [tp / float(numTrials) for tp in totalPops]
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        NoChildException if this virus particle does not reproduce.
        """
        if sum([not self.isResistantTo(drug) for drug in activeDrugs]) == 0 and \
          random.random() < self.getMaxBirthProb() * (1 - popDensity):
            childResistances = self.getResistances().copy()
            for drug, res in childResistances.iteritems():
                if random.random() < self.getMutProb():
                    childResistances[drug] = not res
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    numTimeSteps = 300
    totalPops = [0 for i in range(numTimeSteps)]
    gutResPops = [0 for i in range(numTimeSteps)]
    for i in range(numTrials):
        ...
        for timestep in range(numTimeSteps / 2):
            p.update()
            totalPops[timestep] += p.getTotalPop()
            gutResPops[timestep] += p.getResistPop(["guttagonol"])
        ...
        for timestep in range(numTimeSteps / 2, numTimeSteps):
            ...
            gutResPops[timestep] += p.getResistPop(["guttagonol"])
    avgPops = [tp / float(numTrials) for tp in totalPops]
    avgGutResPops = [tp / float(numTrials) for tp in gutResPops]
    
    pylab.figure(1)
    pylab.plot(avgPops)
    pylab.plot(avgGutResPops)
    pylab.title("Treated with guttagonol after 150")
    pylab.xlabel("Timestep")
    pylab.ylabel("Total virus population")
    pylab.legend("Total", "Resistant to gut")
    pylab.show()
